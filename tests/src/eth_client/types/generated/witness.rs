...
impl ::core::fmt::Debug for Witness { fn fmt(&self, f: &mut ::core::fmt::Formatter) -> ::core::fmt::Result { write!(f, "{}({:#x})", Self::NAME, self) } }
...
impl ::core::fmt::Display for Witness {
    fn fmt(&self, f: &mut ::core::fmt::Formatter) -> ::core::fmt::Result {
        ...
        write!(f, ", {}: {}", "cell_dep_index_list", self.cell_dep_index_list())?;
...
impl ::core::default::Default for Witness {
    fn default() -> Self {
        let v: Vec<u8> = vec![28, 0, 0, 0, 16, 0, 0, 0, 20, 0, 0, 0, 24, 0, 0, 0, 0, 0, 0, 0, 4, 0, 0, 0, 0, 0, 0, 0, ];
...
impl Witness {
    ...
    pub fn merkle_proof(&self) -> BytesVec {
        let slice = self.as_slice();
    ...
    pub fn as_reader<'r>(&'r self) -> WitnessReader<'r> { WitnessReader::new_unchecked(self.as_slice()) }
}

impl molecule::prelude::Entity for Witness {
    type Builder = WitnessBuilder;
    const NAME: &'static str = "Witness";
    fn new_unchecked(data: molecule::bytes::Bytes) -> Self { Witness(data) }
    fn as_bytes(&self) -> molecule::bytes::Bytes { self.0.clone() }
    fn as_slice(&self) -> &[u8] { &self.0[..] }
    fn from_slice(slice: &[u8]) -> molecule::error::VerificationResult<Self> { WitnessReader::from_slice(slice).map(|reader| reader.to_entity()) }
    fn from_compatible_slice(slice: &[u8]) -> molecule::error::VerificationResult<Self> { WitnessReader::from_compatible_slice(slice).map(|reader| reader.to_entity()) }
    fn new_builder() -> Self::Builder { ::core::default::Default::default() }
    fn as_builder(self) -> Self::Builder { Self::new_builder().header(self.header()).merkle_proof(self.merkle_proof()).cell_dep_index_list(self.cell_dep_index_list()) }
}
...
impl<'r> ::core::fmt::Debug for WitnessReader<'r> { fn fmt(&self, f: &mut ::core::fmt::Formatter) -> ::core::fmt::Result { write!(f, "{}({:#x})", Self::NAME, self) } }
...
impl<'r> WitnessReader<'r> {
    ...
    pub fn merkle_proof(&self) -> BytesVecReader<'r> {
        let slice = self.as_slice();
        let start = molecule::unpack_number(&slice[8..]) as usize;
        let end = molecule::unpack_number(&slice[12..]) as usize;
...
impl<'r> molecule::prelude::Reader<'r> for WitnessReader<'r> {
    ...
    fn new_unchecked(slice: &'r [u8]) -> Self { WitnessReader(slice) }
...
pub struct WitnessBuilder { pub(crate) header: Bytes, pub(crate) merkle_proof: BytesVec, pub(crate) cell_dep_index_list: Bytes }
...
impl WitnessBuilder {
    ...
    pub fn merkle_proof(mut self, v: BytesVec) -> Self {
        self.merkle_proof = v;
        self
    }
...
impl molecule::prelude::Builder for WitnessBuilder {
    ...
    fn expected_length(&self) -> usize { molecule::NUMBER_SIZE * (Self::FIELD_COUNT + 1) + self.header.as_slice().len() + self.merkle_proof.as_slice().len() + self.cell_dep_index_list.as_slice().len() }
